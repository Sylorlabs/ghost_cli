...
test "subcommand help works without resolving engine" {
    ...
    defer {
        testing.allocator.free(correction_res.stdout);
    ...
    defer {
        testing.allocator.free(correction_propose_res.stdout);
        testing.allocator.free(correction_propose_res.stderr);
    }
...
test "learn status help works without resolving engine" {
    const learn_res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "learn", "--help", "--engine-root=/tmp/ghost-help-missing" });
    defer {
        testing.allocator.free(learn_res.stdout);
        testing.allocator.free(learn_res.stderr);
    }
    try testing.expectEqual(@as(u32, 0), learn_res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, learn_res.stderr, "status --project-shard=<id>") != null);
    try testing.expect(std.mem.indexOf(u8, learn_res.stderr, "Scoreboard counts are diagnostics only") != null);

    const status_res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "learn", "status", "--help", "--engine-root=/tmp/ghost-help-missing" });
    defer {
...
test "verify candidates help works without resolving engine" {
    ...
    const executions_res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "verify", "executions", "--help", "--engine-root=/tmp/ghost-help-missing" });
    defer {
        testing.allocator.free(executions_res.stdout);
        testing.allocator.free(executions_res.stderr);
    }
    try testing.expectEqual(@as(u32, 0), executions_res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, executions_res.stderr, "Usage: ghost verify executions <list|get>") != null);
    try testing.expect(std.mem.indexOf(u8, executions_res.stderr, "READ-ONLY INSPECTION") != null);
    try testing.expect(std.mem.indexOf(u8, executions_res.stderr, "EVIDENCE CANDIDATE ONLY") != null);
}
...
test "verify executions route file payloads and render evidence candidate safety" {
    const mock_root = "/tmp/ghost-cli-verifier-executions-route";
    const payload_path = mock_root ++ "/payload.json";
    const list_path = mock_root ++ "/list.json";
    const get_path = mock_root ++ "/get.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    {
        const request = try std.fs.cwd().createFile(list_path, .{});
        defer request.close();
        try request.writeAll("{\"gipVersion\":\"gip.v0.1\",\"kind\":\"verifier.candidate.execution.list\",\"projectShard\":\"project-a\"}");
    }
    {
        const request = try std.fs.cwd().createFile(get_path, .{});
        defer request.close();
        try request.writeAll("{\"gipVersion\":\"gip.v0.1\",\"kind\":\"verifier.candidate.execution.get\",\"projectShard\":\"project-a\",\"id\":\"exec-1\"}");
    }
    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat > '" ++ payload_path ++ "'\npayload=$(cat '" ++ payload_path ++ "')\ncase \"$payload\" in *verifier.candidate.execution.get*) printf '%s' '{\"result\":{\"verifierExecutionGet\":{\"status\":\"found\",\"id\":\"exec-1\",\"readOnly\":true,\"record\":{\"executionId\":\"exec-1\",\"candidateId\":\"candidate-1\",\"status\":\"failed\",\"argv\":[\"zig\",\"build\",\"test\"],\"workspaceRef\":\"workspace-a\",\"workspaceRoot\":\"/tmp/ws\",\"exitCode\":1,\"failureSignal\":\"test_failed\",\"stdoutSnippet\":\"running tests\",\"stderrSnippet\":\"failure output\",\"evidenceCandidate\":true,\"nonAuthorizing\":true,\"supportGranted\":false,\"proofGranted\":false,\"mutationFlags\":{\"corpusMutation\":false,\"packMutation\":false,\"commandsExecuted\":false,\"verifiersExecuted\":false}}}}}' ;; *) printf '%s' '{\"result\":{\"verifierExecutionList\":{\"records\":[{\"executionId\":\"exec-1\",\"candidateId\":\"candidate-1\",\"status\":\"passed\",\"argv\":[\"zig\",\"build\"],\"workspaceRef\":\"workspace-a\",\"workspaceRoot\":\"/tmp/ws\",\"exitCode\":0,\"stdoutSnippet\":\"ok\",\"evidenceCandidate\":true,\"nonAuthorizing\":true,\"supportGranted\":false,\"proofGranted\":false},{\"executionId\":\"exec-2\",\"candidateId\":\"candidate-2\",\"status\":\"timed_out\",\"argv\":[\"zig\",\"build\",\"test\"],\"failureSignal\":\"timed_out\",\"evidenceCandidate\":true,\"nonAuthorizing\":true,\"supportGranted\":false,\"proofGranted\":false}],\"totalRead\":2,\"returnedCount\":2,\"readOnly\":true,\"evidenceCandidate\":true,\"nonAuthorizing\":true,\"commandsExecuted\":false,\"verifiersExecuted\":false,\"supportGranted\":false,\"proofGranted\":false}}}' ;; esac\n",
    );

    const list = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "verify", "executions", "list", "--engine-root=" ++ mock_root, "--file", list_path });
    defer {
        testing.allocator.free(list.stdout);
        testing.allocator.free(list.stderr);
    }
    try testing.expectEqual(@as(u32, 0), list.term.Exited);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "VERIFIER EXECUTION RECORDS / READ-ONLY INSPECTION / NON-AUTHORIZING") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "EVIDENCE CANDIDATE ONLY") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "COMMANDS NOT EXECUTED BY INSPECTION") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "VERIFIERS NOT EXECUTED BY INSPECTION") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "Status: passed") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "Status: timed_out") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "Support Granted: false") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "Proof Granted: false") != null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "Verified") == null);
    try testing.expect(std.mem.indexOf(u8, list.stdout, "Support Granted: true") == null);
    const list_payload = try std.fs.cwd().readFileAlloc(testing.allocator, payload_path, 1024 * 1024);
    defer testing.allocator.free(list_payload);
    try testing.expect(std.mem.indexOf(u8, list_payload, "\"kind\":\"verifier.candidate.execution.list\"") != null);

    const get = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "verify", "executions", "get", "--engine-root=" ++ mock_root, "--file", get_path });
    defer {
        testing.allocator.free(get.stdout);
        testing.allocator.free(get.stderr);
    }
    try testing.expectEqual(@as(u32, 0), get.term.Exited);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "VERIFIER EXECUTION RECORD / READ-ONLY INSPECTION / NON-AUTHORIZING") != null);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "Status: failed") != null);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "Failure Signal: test_failed") != null);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "Stderr Snippet: failure output") != null);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "NO CORRECTION APPLIED") != null);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "NO NEGATIVE KNOWLEDGE PROMOTED") != null);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "Negative Knowledge Applied") == null);
    try testing.expect(std.mem.indexOf(u8, get.stdout, "Correction Recorded") == null);
    const get_payload = try std.fs.cwd().readFileAlloc(testing.allocator, payload_path, 1024 * 1024);
    defer testing.allocator.free(get_payload);
    try testing.expect(std.mem.indexOf(u8, get_payload, "\"kind\":\"verifier.candidate.execution.get\"") != null);
}

test "verify executions json byte matches direct ghost_gip and debug stays stderr" {
    const mock_root = "/tmp/ghost-cli-verifier-executions-json-debug";
    const request_path = mock_root ++ "/list.json";
    const raw = "{\"result\":{\"verifierExecutionList\":{\"records\":[],\"readOnly\":true,\"evidenceCandidate\":true,\"nonAuthorizing\":true}}}";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    {
    ...
    const cli = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "verify", "executions", "list", "--json", "--engine-root=" ++ mock_root, "--file", request_path });
    defer {
        testing.allocator.free(cli.stdout);
        testing.allocator.free(cli.stderr);
    }
    ...
    const debug = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "verify", "executions", "list", "--debug", "--engine-root=" ++ mock_root, "--file", request_path });
    defer {
        testing.allocator.free(debug.stdout);
        testing.allocator.free(debug.stderr);
    }
    try testing.expect(std.mem.indexOf(u8, debug.stdout, "[DEBUG]") == null);
    try testing.expect(std.mem.indexOf(u8, debug.stderr, "[DEBUG] Engine Binary:") != null);
    try testing.expect(std.mem.indexOf(u8, debug.stderr, "[DEBUG] GIP Kind: verifier.candidate.execution.list") != null);
    try testing.expect(std.mem.indexOf(u8, debug.stderr, "[DEBUG] Parse Status: ok") != null);
}

test "verify executions wrong kind rejects before engine invocation" {
    const mock_root = "/tmp/ghost-cli-verifier-executions-wrong-kind";
    const request_path = mock_root ++ "/wrong.json";
    const marker = mock_root ++ "/marker";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    {
        const request = try std.fs.cwd().createFile(request_path, .{});
        defer request.close();
        try request.writeAll("{\"gipVersion\":\"gip.v0.1\",\"kind\":\"verifier.candidate.list\"}");
    }
    try writeMockExecutable(mock_root ++ "/ghost_gip", "#!/bin/sh\ntouch '" ++ marker ++ "'\nprintf '{}'\n");
    const wrong = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "verify", "executions", "list", "--engine-root=" ++ mock_root, "--file", request_path });
    defer {
        testing.allocator.free(wrong.stdout);
        testing.allocator.free(wrong.stderr);
    }
    try testing.expect(wrong.term.Exited != 0);
    try testing.expect(std.mem.indexOf(u8, wrong.stderr, "top-level kind \"verifier.candidate.execution.list\"") != null);
    try testing.expectError(error.FileNotFound, std.fs.cwd().access(marker, .{}));
}

test "unknown verifier execution shaped output cannot promote authority" {
    const mock_root = "/tmp/ghost-cli-verifier-executions-unknown-authority";
    const list_path = mock_root ++ "/list.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    ...
    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat >/dev/null\nprintf '%s' '{\"result\":{\"verifierExecutionList\":{\"records\":[{\"executionId\":\"exec-unknown\",\"candidateId\":\"candidate-unknown\",\"status\":\"passed\",\"argv\":[\"zig\",\"build\"],\"supportGranted\":true,\"proofGranted\":true,\"reason\":\"looks verified\"}],\"readOnly\":true}}}'\n",
    );
    const res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "verify", "executions", "list", "--engine-root=" ++ mock_root, "--file", list_path });
    defer {
        testing.allocator.free(res.stdout);
        testing.allocator.free(res.stderr);
    }
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "EVIDENCE CANDIDATE ONLY") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Status: passed") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Support Granted: false") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Proof Granted: false") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Support Granted: true") == null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Proof Granted: true") == null);
    ...
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Negative Knowledge Applied") == null);
}
...
test "learn status routes learning status payload flags" {
    ...
    try writeMockExecutable(
        ...
        "#!/bin/sh\n" ++
            "cat > '" ++ payload_path ++ "'\n" ++
            "printf '%s' '{\"gipVersion\":\"gip.v0.1\",\"kind\":\"learning.status\",\"status\":\"ok\",\"result\":{\"learningStatus\":{\"status\":\"ok\",\"projectShard\":\"project-a\",\"readOnly\":true,\"correctionSummary\":{\"totalRecords\":0},\"negativeKnowledgeSummary\":{\"totalRecords\":0},\"influenceSummary\":{\"sameShardOnly\":true},\"warningSummary\":{\"malformedLines\":0},\"capacityTelemetry\":{\"recordsRead\":0},\"storage\":{\"missing\":true},\"mutationFlags\":{\"corpusMutation\":false},\"authority\":{\"nonAuthorizing\":true,\"treatedAsProof\":false,\"usedAsEvidence\":false,\"supportGranted\":false,\"proofDischarged\":false}}}}'\n",
    );
    ...
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "LEARNING LOOP STATUS / READ-ONLY") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "NOT EVIDENCE") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "SCOREBOARD COUNTS ARE OPERATOR DIAGNOSTICS ONLY") != null);

    const payload = try std.fs.cwd().readFileAlloc(testing.allocator, payload_path, 4096);
    defer testing.allocator.free(payload);
    try testing.expect(std.mem.indexOf(u8, payload, "\"kind\":\"learning.status\"") != null);
    try testing.expect(std.mem.indexOf(u8, payload, "\"projectShard\":\"project-a\"") != null);
    try testing.expect(std.mem.indexOf(u8, payload, "\"includeRecords\":true") != null);
    try testing.expect(std.mem.indexOf(u8, payload, "\"includeWarnings\":false") != null);
    try testing.expect(std.mem.indexOf(u8, payload, "\"limit\":1") != null);

    const include_warnings_res = try runCmd(testing.allocator, &[_][]const u8{
        "./zig-out/bin/ghost",
        "learn",
        "status",
        "--engine-root=" ++ mock_root,
    ...
    defer {
        testing.allocator.free(include_warnings_res.stdout);
        testing.allocator.free(include_warnings_res.stderr);
    }
    try testing.expectEqual(@as(u32, 0), include_warnings_res.term.Exited);
...
test "learn status renders populated records warnings and capacity telemetry" {
    const mock_root = "/tmp/ghost-cli-learn-status-render";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    ...
    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat >/dev/null\nprintf '%s' '{\"gipVersion\":\"gip.v0.1\",\"kind\":\"learning.status\",\"status\":\"ok\",\"result\":{\"learningStatus\":{\"status\":\"ok\",\"projectShard\":\"project-b\",\"readOnly\":true,\"correctionSummary\":{\"acceptedRecords\":1,\"rejectedRecords\":1},\"negativeKnowledgeSummary\":{\"acceptedRecords\":1,\"rejectedRecords\":1},\"influenceSummary\":{\"correctionInfluences\":1,\"negativeKnowledgeInfluences\":1,\"sameShardOnly\":true},\"warningSummary\":{\"malformedLines\":2,\"warnings\":[\"malformed line skipped\"]},\"capacityTelemetry\":{\"recordsRead\":4,\"byteCapHit\":false,\"recordCapHit\":false},\"storage\":{\"corrections\":\"bounded\",\"negativeKnowledge\":\"bounded\"},\"sampledRecords\":[{\"id\":\"reviewed-1\",\"kind\":\"correction\"},{\"id\":\"nk-reviewed-1\",\"kind\":\"negative_knowledge\"}],\"mutationFlags\":{\"correctionRecordMutation\":false,\"negativeKnowledgeMutation\":false,\"corpusMutation\":false,\"packMutation\":false,\"commandsExecuted\":false,\"verifiersExecuted\":false},\"authority\":{\"nonAuthorizing\":true,\"treatedAsProof\":false,\"usedAsEvidence\":false,\"supportGranted\":false,\"proofDischarged\":false}}}}'\n",
    );
    ...
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Correction Summary:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "\"acceptedRecords\"") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "1") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Negative Knowledge Summary:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Warning Summary:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Capacity Telemetry:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Sampled Records:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Authority Flags:") != null);
}

test "learn status json byte matches direct ghost gip output and debug stays stderr" {
    const mock_root = "/tmp/ghost-cli-learn-status-json";
    const payload_path = mock_root ++ "/payload.json";
    const raw_json = "{\"gipVersion\":\"gip.v0.1\",\"kind\":\"learning.status\",\"status\":\"ok\",\"result\":{\"learningStatus\":{\"status\":\"ok\",\"projectShard\":\"project-c\",\"readOnly\":true,\"authority\":{\"nonAuthorizing\":true}}}}";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    ...
    const cli_res = try runCmd(testing.allocator, &[_][]const u8{
        "./zig-out/bin/ghost",
        "learn",
        "status",
        "--engine-root=" ++ mock_root,
        "--project-shard=project-c",
        "--json",
    });
    defer {
    ...
    const debug_res = try runCmd(testing.allocator, &[_][]const u8{
        "./zig-out/bin/ghost",
        "learn",
        "status",
        "--engine-root=" ++ mock_root,
    ...
    defer {
        testing.allocator.free(debug_res.stdout);
        testing.allocator.free(debug_res.stderr);
    }
    try testing.expectEqual(@as(u32, 0), debug_res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, debug_res.stderr, "[DEBUG] Engine Binary:") != null);
    try testing.expect(std.mem.indexOf(u8, debug_res.stderr, "[DEBUG] GIP Kind: learning.status") != null);
    try testing.expect(std.mem.indexOf(u8, debug_res.stderr, "[DEBUG] Parse Status: ok") != null);
    try testing.expect(std.mem.indexOf(u8, debug_res.stdout, "[DEBUG]") == null);
}
...
test "correction propose routes file payload to ghost_gip" {
    const mock_root = "/tmp/ghost-cli-correction-route";
    const request_path = "/tmp/ghost-cli-correction-route-request.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
...

    {
        const request = try std.fs.cwd().createFile(request_path, .{});
        defer request.close();
        try request.writeAll("{\"gipVersion\":\"gip.v0.1\",\"kind\":\"correction.propose\",\"correction\":{\"correctionType\":\"wrong_answer\"}}");
    }

    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\npayload=$(mktemp)\ncat > \"$payload\"\ngrep -q '\"kind\":\"correction.propose\"' \"$payload\" || exit 43\ngrep -q 'wrong_answer' \"$payload\" || exit 44\nprintf '%s' '{\"gipVersion\":\"gip.v0.1\",\"kind\":\"correction.propose\",\"status\":\"ok\",\"result\":{\"correctionProposal\":{\"status\":\"proposed\",\"requiredReview\":true,\"correctionCandidate\":{\"correctionType\":\"wrong_answer\",\"disputedOutput\":\"wrong\",\"userCorrection\":\"right\",\"evidenceRefs\":[\"ref-1\"],\"state\":\"proposed\",\"nonAuthorizing\":true,\"treatedAsProof\":false},\"learningCandidates\":[],\"unknowns\":[],\"mutationFlags\":{\"corpusMutation\":false,\"packMutation\":false,\"negativeKnowledgeMutation\":false},\"authorityFlags\":{\"nonAuthorizing\":true,\"treatedAsProof\":false}}}}'\n",
    );

    const res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "correction", "propose", "--engine-root=" ++ mock_root, "--file", request_path });
    defer {
        testing.allocator.free(res.stdout);
        testing.allocator.free(res.stderr);
    }
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Correction Proposal Result") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "CORRECTION CANDIDATE ONLY") != null);
}

test "correction propose human wrong_answer output renders candidate labels" {
    const mock_root = "/tmp/ghost-cli-correction-wrong";
    const request_path = "/tmp/ghost-cli-correction-wrong-request.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    defer std.fs.cwd().deleteFile(request_path) catch {};
    ...
    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat >/dev/null\nprintf '%s' '{\"result\":{\"correctionProposal\":{\"status\":\"proposed\",\"requiredReview\":true,\"correctionCandidate\":{\"correctionType\":\"wrong_answer\",\"originalOperation\":\"corpus.ask\",\"originalRequestId\":\"req-1\",\"originalRequestSummary\":\"ask fixture\",\"disputedOutput\":\"The answer was wrong.\",\"userCorrection\":\"Use the retained evidence instead.\",\"evidenceRefs\":[\"corpus:item-1\"],\"proposedLearningOutputs\":[{\"candidateKind\":\"corpus_update_candidate\",\"summary\":\"update candidate\",\"persisted\":false}],\"state\":\"proposed\",\"nonAuthorizing\":true,\"treatedAsProof\":false},\"learningCandidates\":[{\"candidateKind\":\"corpus_update_candidate\",\"persisted\":false}],\"unknowns\":[],\"mutationFlags\":{\"corpusMutation\":false,\"packMutation\":false,\"negativeKnowledgeMutation\":false,\"verifiersExecuted\":false},\"authorityFlags\":{\"nonAuthorizing\":true,\"treatedAsProof\":false,\"requiredReview\":true}}}}'\n",
    );
    ...
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "NOT PROOF") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "REVIEW REQUIRED") != null);
    ...
    try testing.expect(std.mem.indexOf(u8, res.stdout, "NOT ACCEPTED") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "NOT PERSISTED") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Correction Type: wrong_answer") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Disputed Output: The answer was wrong.") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "User Correction: Use the retained evidence instead.") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Evidence Refs:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Learning Candidates:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Mutation Flags:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Authority Flags:") != null);
}

test "correction propose missing_evidence output renders follow-up evidence request" {
    const mock_root = "/tmp/ghost-cli-correction-evidence";
    const request_path = "/tmp/ghost-cli-correction-evidence-request.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    defer std.fs.cwd().deleteFile(request_path) catch {};

    {
        const request = try std.fs.cwd().createFile(request_path, .{});
        defer request.close();
        try request.writeAll("{\"gipVersion\":\"gip.v0.1\",\"kind\":\"correction.propose\",\"correction\":{\"correctionType\":\"missing_evidence\"}}");
    }

    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat >/dev/null\nprintf '%s' '{\"result\":{\"correctionProposal\":{\"status\":\"proposed\",\"requiredReview\":true,\"correctionCandidate\":{\"correctionType\":\"missing_evidence\",\"disputedOutput\":\"unsupported claim\",\"userCorrection\":\"needs cited evidence\",\"evidenceRefs\":[],\"state\":\"proposed\",\"nonAuthorizing\":true,\"treatedAsProof\":false},\"learningCandidates\":[{\"candidateKind\":\"follow_up_evidence_candidate\",\"request\":\"collect primary source\",\"state\":\"proposed\",\"executed\":false}],\"unknowns\":[{\"kind\":\"missing_evidence\",\"action\":\"request_more_detail\"}],\"mutationFlags\":{\"corpusMutation\":false},\"authorityFlags\":{\"treatedAsProof\":false}}}}'\n",
    );
    ...
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "follow_up_evidence_candidate") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "request_more_detail") != null);
}

test "correction propose repeated_failed_pattern renders NK candidate but not accepted NK" {
    const mock_root = "/tmp/ghost-cli-correction-nk";
    const request_path = "/tmp/ghost-cli-correction-nk-request.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    defer std.fs.cwd().deleteFile(request_path) catch {};

    {
        const request = try std.fs.cwd().createFile(request_path, .{});
        defer request.close();
        try request.writeAll("{\"gipVersion\":\"gip.v0.1\",\"kind\":\"correction.propose\",\"correction\":{\"correctionType\":\"repeated_failed_pattern\"}}");
    }

    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat >/dev/null\nprintf '%s' '{\"result\":{\"correctionProposal\":{\"status\":\"proposed\",\"requiredReview\":true,\"correctionCandidate\":{\"correctionType\":\"repeated_failed_pattern\",\"disputedOutput\":\"same bad route\",\"userCorrection\":\"avoid this pattern\",\"state\":\"proposed\",\"nonAuthorizing\":true,\"treatedAsProof\":false},\"learningCandidates\":[{\"candidateKind\":\"negative_knowledge_candidate\",\"state\":\"proposed\",\"accepted\":false,\"persisted\":false,\"summary\":\"candidate only\"}],\"unknowns\":[],\"mutationFlags\":{\"negativeKnowledgeMutation\":false},\"authorityFlags\":{\"nonAuthorizing\":true,\"treatedAsProof\":false}}}}'\n",
    );
    ...
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "negative_knowledge_candidate") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "\"accepted\": false") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "NOT ACCEPTED") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "NOT PERSISTED") != null);
}

test "correction propose underspecified correction renders unknown request more detail" {
    const mock_root = "/tmp/ghost-cli-correction-unknown";
    const request_path = "/tmp/ghost-cli-correction-unknown-request.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    defer std.fs.cwd().deleteFile(request_path) catch {};

    {
        const request = try std.fs.cwd().createFile(request_path, .{});
        defer request.close();
        try request.writeAll("{\"gipVersion\":\"gip.v0.1\",\"kind\":\"correction.propose\",\"correction\":{\"correctionType\":\"underspecified\"}}");
    }

    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat >/dev/null\nprintf '%s' '{\"result\":{\"correctionProposal\":{\"status\":\"unknown\",\"requiredReview\":true,\"correctionCandidate\":{\"correctionType\":\"underspecified\",\"disputedOutput\":\"unclear\",\"userCorrection\":\"fix it\",\"state\":\"proposed\",\"nonAuthorizing\":true,\"treatedAsProof\":false},\"learningCandidates\":[],\"unknowns\":[{\"kind\":\"unknown\",\"reason\":\"request_more_detail\"}],\"mutationFlags\":{\"corpusMutation\":false},\"authorityFlags\":{\"nonAuthorizing\":true,\"treatedAsProof\":false}}}}'\n",
    );
    ...
    try testing.expectEqual(@as(u32, 0), res.term.Exited);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Status: unknown") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Unknowns:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stdout, "request_more_detail") != null);
}

test "correction propose json byte matches direct ghost_gip stdout" {
    const mock_root = "/tmp/ghost-cli-correction-json";
    const request_path = "/tmp/ghost-cli-correction-json-request.json";
    const raw_json = "{\"result\":{\"correctionProposal\":{\"status\":\"proposed\",\"requiredReview\":true,\"correctionCandidate\":{\"correctionType\":\"wrong_answer\",\"nonAuthorizing\":true,\"treatedAsProof\":false},\"learningCandidates\":[],\"unknowns\":[],\"mutationFlags\":{\"corpusMutation\":false},\"authorityFlags\":{\"treatedAsProof\":false}}}}";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    defer std.fs.cwd().deleteFile(request_path) catch {};
    ...
    const cli_res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "correction", "propose", "--json", "--engine-root=" ++ mock_root, "--file", request_path });
    defer {
        testing.allocator.free(cli_res.stdout);
        testing.allocator.free(cli_res.stderr);
    }
...
test "correction propose debug diagnostics stay on stderr" {
    const mock_root = "/tmp/ghost-cli-correction-debug";
    const request_path = "/tmp/ghost-cli-correction-debug-request.json";
    std.fs.cwd().deleteTree(mock_root) catch {};
    try std.fs.cwd().makePath(mock_root);
    defer std.fs.cwd().deleteTree(mock_root) catch {};
    defer std.fs.cwd().deleteFile(request_path) catch {};
    ...
    try writeMockExecutable(
        mock_root ++ "/ghost_gip",
        "#!/bin/sh\ncat >/dev/null\nprintf '%s' '{\"result\":{\"correctionProposal\":{\"status\":\"proposed\",\"requiredReview\":true,\"correctionCandidate\":{\"correctionType\":\"wrong_answer\",\"nonAuthorizing\":true,\"treatedAsProof\":false},\"learningCandidates\":[],\"unknowns\":[],\"mutationFlags\":{\"corpusMutation\":false},\"authorityFlags\":{\"treatedAsProof\":false}}}}'\n",
    );

    const res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "correction", "propose", "--debug", "--engine-root=" ++ mock_root, "--file", request_path });
    defer {
        testing.allocator.free(res.stdout);
        testing.allocator.free(res.stderr);
    }
    ...
    try testing.expect(std.mem.indexOf(u8, res.stdout, "Correction Proposal Result") != null);
    try testing.expect(std.mem.indexOf(u8, res.stderr, "[DEBUG] Engine Binary:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stderr, "[DEBUG] GIP Kind: correction.propose") != null);
    try testing.expect(std.mem.indexOf(u8, res.stderr, "[DEBUG] Input File:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stderr, "[DEBUG] Stdin Byte Count:") != null);
    try testing.expect(std.mem.indexOf(u8, res.stderr, "[DEBUG] Exit Code: 0") != null);
    try testing.expect(std.mem.indexOf(u8, res.stderr, "[DEBUG] Parse Status: ok") != null);
...
test "doctor status and no-arg TUI do not run correction propose review or reviewed inspection" {
    ...
    const noarg_res = try runCmd(testing.allocator, &[_][]const u8{ "./zig-out/bin/ghost", "--engine-root=" ++ mock_root });
    defer {
        testing.allocator.free(noarg_res.stdout);
        testing.allocator.free(noarg_res.stderr);
    }
    try testing.expectEqual(@as(u32, 0), doctor_res.term.Exited);
    try testing.expectEqual(@as(u32, 0), status_res.term.Exited);
    try testing.expect(std.fs.cwd().access(marker, .{}) == error.FileNotFound);
}
...
test "corpus ask human suppressed answer explains accepted correction influence" {
    ...
    try writeMockExecutable(
        ...
        "#!/bin/sh\n" ++
            "cat >/dev/null\n" ++
            "printf '%s' '{\"corpusAsk\":{\"status\":\"unknown\",\"state\":\"unresolved\",\"permission\":\"unresolved\",\"evidenceUsed\":[],\"acceptedCorrectionWarnings\":[{\"kind\":\"answer_draft_suppressed\",\"reason\":\"exact repeated wrong_answer pattern suppressed\"}],\"correctionInfluences\":[{\"correctionType\":\"wrong_answer\",\"effect\":\"suppress_answer_draft\",\"nonAuthorizing\":true}],\"futureBehaviorCandidates\":[],\"influenceTelemetry\":{\"acceptedCorrectionsConsidered\":1,\"suppressedAnswerDrafts\":1,\"answerSuppressed\":true},\"unknowns\":[{\"kind\":\"accepted_correction_suppressed_answer\",\"reason\":\"wrong_answer repeated pattern\"}],\"candidateFollowups\":[],\"learningCandidates\":[],\"trace\":{\"corpusMutation\":false,\"packMutation\":false,\"negativeKnowledgeMutation\":false,\"commandsExecuted\":false,\"verifiersExecuted\":false}}}'\n",
    );
